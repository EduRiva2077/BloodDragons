...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-stone-950 flex flex-col items-center justify-center p-4 relative overflow-hidden">
      <!-- Background Effects -->
      <div class="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-red-900/20 via-stone-950 to-stone-950 pointer-events-none"></div>
      ...
      <div class="relative z-10 w-full max-w-4xl flex flex-col items-center">
        ...
        <div class="text-center mb-16 animate-fade-in-up">
          <h1 class="text-6xl md:text-8xl font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-b from-stone-200 to-stone-500 drop-shadow-2xl mb-4" style="font-family: 'Playfair Display', serif;">
            D&D
          </h1>
          <h2 class="text-4xl md:text-6xl font-bold tracking-widest uppercase text-transparent bg-clip-text bg-gradient-to-r from-stone-400 via-red-500 to-amber-500 drop-shadow-lg" style="font-family: 'Cinzel', serif;">
            The Elden BloodMoon
          </h2>
          <div class="h-1 w-32 bg-gradient-to-r from-transparent via-red-600 to-transparent mx-auto mt-6"></div>
        </div>
        ...
        @if (view() === 'menu') {
          <div class="flex flex-col md:flex-row gap-6 w-full max-w-2xl animate-fade-in">
            ...
            <button (click)="view.set('create')" class="flex-1 group relative overflow-hidden rounded-xl border border-stone-800 bg-stone-900/50 p-8 hover:bg-stone-800/80 hover:border-red-900/50 transition-all duration-500 text-left">
              ...
              <mat-icon class="text-red-500 mb-4 transform group-hover:scale-110 transition-transform duration-500" style="font-size: 48px; width: 48px; height: 48px;">add_circle_outline</mat-icon>
            ...
            <button (click)="view.set('load')" class="flex-1 group relative overflow-hidden rounded-xl border border-stone-800 bg-stone-900/50 p-8 hover:bg-stone-800/80 hover:border-amber-900/50 transition-all duration-500 text-left">
              <div class="absolute inset-0 bg-gradient-to-br from-amber-900/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
              <mat-icon class="text-amber-500 mb-4 transform group-hover:scale-110 transition-transform duration-500" style="font-size: 48px; width: 48px; height: 48px;">auto_stories</mat-icon>
        ...
        @if (view() === 'create') {
          <div class="w-full max-w-md bg-stone-900/80 backdrop-blur-md border border-stone-800 rounded-xl p-8 animate-fade-in shadow-2xl">
            <h3 class="text-2xl font-bold text-stone-200 mb-6 font-serif flex items-center gap-3">
              <mat-icon class="text-red-500">add_circle</mat-icon>
              Nova Campanha
            </h3>
            ...
            <div class="mb-6">
              ...
              <input type="text" [(ngModel)]="newCampaignName" placeholder="Ex: A Maldição de Strahd" 
                     class="w-full bg-stone-950 border border-stone-700 rounded-lg px-4 py-3 text-stone-200 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 transition-all"
            ...
            <div class="flex gap-3">
              ...
              <button (click)="createCampaign()" [disabled]="!newCampaignName.trim()" 
                      class="flex-1 px-4 py-3 rounded-lg bg-red-900/80 text-red-100 hover:bg-red-800 border border-red-700 transition-colors font-bold disabled:opacity-50 disabled:cursor-not-allowed">
                Criar
              </button>
            </div>
          </div>
          ...
          <div class="w-full max-w-2xl bg-stone-900/80 backdrop-blur-md border border-stone-800 rounded-xl p-8 animate-fade-in shadow-2xl">
            <div class="flex items-center justify-between mb-6">
              <h3 class="text-2xl font-bold text-stone-200 font-serif flex items-center gap-3">
                <mat-icon class="text-amber-500">auto_stories</mat-icon>
                Saves
              </h3>
            ...
            <div class="space-y-3 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
              @for (campaign of campaignService.campaigns(); track campaign.id) {
                <button (click)="loadCampaign(campaign.id)" class="w-full text-left bg-stone-950 border border-stone-800 rounded-lg p-4 hover:border-amber-500/50 hover:bg-stone-800/50 transition-all group flex items-center justify-between">
                  <div>
                    <h4 class="text-lg font-bold text-stone-200 group-hover:text-amber-400 transition-colors">{{ campaign.name }}</h4>
                  ...
                  <mat-icon class="text-stone-600 group-hover:text-amber-500 transition-colors">play_circle_filled</mat-icon>
                </button>
              }
  ...
  `,
  styles: [`
    @import url('https://fonts.googleapis.com/css2?family=Cinzel:wght@700&family=Playfair+Display:ital,wght@0,900;1,900&display=swap');
